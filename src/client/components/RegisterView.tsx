  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ email, password, enrollmentId }),
      });
    ...
    <div>
      ...
      <main className="bg-white rounded-2xl p-5 sm:p-6">
        <form onSubmit={handleSubmit}>
          ...
          <label htmlFor="eid" className="block text-xs font-semibold text-slate-600 mb-1.5">
            Enrollment ID
          </label>
          <input
            type="text"
            id="eid"
            value={enrollmentId}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEnrollmentId(e.target.value)}
            placeholder="e.g. 2633040537"
            required
            maxLength={100}
            autoComplete="off"
        ...
        <p className="mt-4 text-center text-xs text-slate-400">
          ...
          <button type="button" onClick={onSwitchToLogin} className="text-blue-600 font-semibold border-none bg-transparent cursor-pointer">
            Sign in
          </button>
        </p>
      </main>
